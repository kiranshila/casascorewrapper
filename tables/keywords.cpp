...
template <typename T> T getKeyword(Table *t, char const *keyword) {
...
template <typename T> void putKeyword(Table *t, char const *keyword, T input) {
  // Note that it is very important that `keywords` is a reference here.
  // Otherwise we will make a copy of the `TableRecord` and any changes will
  // fail to propagate back to the table.
...
template <typename T>
void putKeyword_array(TableRecord &keywords, char const *keyword, T *input,
                      int const *dims, int ndim) {
  keywords.define(keyword, *input_array(input, dims, ndim));
}

template <typename T>
void putKeyword_array(Table *t, char const *keyword, T *input, int const *dims,
                      int ndim) {
  // Note that it is very important that `keywords` is a reference here.
  // Otherwise we will make a copy of the `TableRecord` and any changes will
  // fail to propagate back to the table.
...
void put_keyword_array_boolean(Table *t, char *keyword, bool *input, int *dims,
                               int ndim) {
  putKeyword_array(t, keyword, input, dims, ndim);
}
void put_keyword_array_int(Table *t, char *keyword, int *input, int *dims,
                           int ndim) {
  putKeyword_array(t, keyword, input, dims, ndim);
}
void put_keyword_array_float(Table *t, char *keyword, float *input, int *dims,
                             int ndim) {
  putKeyword_array(t, keyword, input, dims, ndim);
}
void put_keyword_array_double(Table *t, char *keyword, double *input, int *dims,
                              int ndim) {
  putKeyword_array(t, keyword, input, dims, ndim);
}
...
void put_column_keyword_boolean(Table *t, char *column, char *keyword,
                                bool input) {
...
void put_column_keyword_float(Table *t, char *column, char *keyword,
                              float input) {
  putKeyword<Float>(t, column, keyword, input);
}
void put_column_keyword_double(Table *t, char *column, char *keyword,
                               double input) {
