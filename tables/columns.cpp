...
template <typename T, typename R> R *getColumn(Table *t, char const *name) {
...
template <typename T> T *getColumn(Table *t, char const *name) {
  return getColumn<T, T>(t, name);
}

template <typename T, typename R>
void putColumn(Table *t, char const *name, R const *input, int const *dims,
               int ndim) {
  auto table_description = t->tableDesc();
  auto column_description = table_description.columnDesc(name);
  if (column_description.isScalar()) {
...
template <typename T>
void putColumn(Table *t, char const *name, T const *input, int const *dims,
               int ndim) {
  putColumn<T, T>(t, name, input, dims, ndim);
}

extern "C" {
uint num_columns(Table *t) { return t->tableDesc().ncolumn(); }
...
void remove_column(Table *t, char *columnName) { t->removeColumn(columnName); }

// get/put columns

bool column_is_fixed_shape(Table *t, char *name) {
  ROTableColumn col(*t, name);
  return (col.columnDesc().options() & ColumnDesc::FixedShape) ==
    ...
    *dimension = 1;
    ...
    if (column_is_fixed_shape(t, name)) {
      ...
      *dimension = colshape.size() + 1;
      ...
      // if the column doesn't have a fixed shape, we will rely on the shape of
      // the array in the first row to get the shape of the entire column, but
      // we need to be sure that the first row actually has an array in it (ie.
      // it's not left undefined).
      if (col.isDefined(0)) {
...
int *get_column_int(Table *t, char *name) { return getColumn<Int>(t, name); }
...
void put_column_boolean(Table *t, char *name, bool *input, int *dims,
                        int ndim) {
...
void put_column_double(Table *t, char *name, double *input, int *dims,
                       int ndim) {
  putColumn(t, name, input, dims, ndim);
}
