...
//
// NOTE: Templated functions need to be defined here. Regular functions should
// be defined in util.cpp.
...
template <typename T> T *output_array(Array<T> const &array) {
  ...
  if (array.contiguousStorage()) {
    ...
    // This branch is relatively untested because I'm not entirely sure how to
    // create a non-contiguous array. The casacore documentation assures me that
...
unique_ptr<Array<String>> input_array(char *const *input, int const *dims,
                                      int ndim);
